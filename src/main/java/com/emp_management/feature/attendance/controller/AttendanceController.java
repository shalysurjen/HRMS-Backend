...
public class AttendanceController {
    ...
    public ResponseEntity<List<AttendanceMonthlySummaryDTO>> getMonthlySummaryAll(
            @RequestParam @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate fromDate,
            @RequestParam @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate toDate) {

        List<AttendanceMonthlySummaryDTO> data = service.getMonthlySummaryAll(fromDate, toDate);
        return ResponseEntity.ok(data);
    }

    // ─── Monthly Summary: Team only (Manager) ──────────────────────
    @PostMapping("/summary/team")
    public ResponseEntity<List<AttendanceMonthlySummaryDTO>> getMonthlySummaryTeam(
            @RequestBody AttendanceExportRequest request) {

        List<AttendanceMonthlySummaryDTO> data = service.getMonthlySummaryForTeam(
                request.getEmpIds(), request.getFromDate(), request.getToDate());
        return ResponseEntity.ok(data);
    }

    // ─── Excel Download: Monthly Summary All ───────────────────────
    @GetMapping("/summary/download/all")
    public ResponseEntity<InputStreamResource> downloadMonthlySummaryAll(
            @RequestParam @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate fromDate,
            @RequestParam @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate toDate) {

        List<AttendanceMonthlySummaryDTO> data = service.getMonthlySummaryAll(fromDate, toDate);
        ByteArrayInputStream in = service.exportMonthlySummaryToExcel(data, fromDate, toDate);
        String fileName = "Monthly_Attendance_Summary_" + fromDate + "_to_" + toDate + ".xlsx";
    ...
    // ─── Excel Download: Team Summary (Manager) ────────────────────
    @PostMapping("/summary/download/team")
    public ResponseEntity<InputStreamResource> downloadMonthlySummaryTeam(
            @RequestBody AttendanceExportRequest request) {

        List<AttendanceMonthlySummaryDTO> data = service.getMonthlySummaryForTeam(
                request.getEmpIds(), request.getFromDate(), request.getToDate());
        ByteArrayInputStream in = service.exportMonthlySummaryToExcel(data, request.getFromDate(), request.getToDate());
        String fileName = "Team_Attendance_Summary_" + request.getFromDate() + "_to_" + request.getToDate() + ".xlsx";
